...
public class Level5 extends LevelState{
    ...
    private boolean tubeLimiter;
    private Array<EnemyBird> enemies;
    private Music kill;

    public Level5(GameStateManager gsm) {
        super(gsm);
        bird = new PlayerBird(100,250, 10);
        background = new Texture("texture8.png");
    ...
    public void update(float dt) {
        ...
        for(int i = 0; i < tubes.size; i++){
            ...
            if (i % 2 == 0){
                if ((tube.getPosTopTube().y) == 320){
                    tubeLimiter = false;
                }
                else if ((tube.getPosTopTube().y) == 180){
                    tubeLimiter = true;
                }
                if (tubeLimiter){
                    tube.setPosBotTube(tube.getPosBotTube().y + 1);
                    tube.setPosTopTube(tube.getPosTopTube().y + 1);
                }
                else{
                    tube.setPosBotTube(tube.getPosBotTube().y - 1);
                    tube.setPosTopTube(tube.getPosTopTube().y - 1);
        ...
        if(levelCompleted){
            gsm.set(new VictoryState(gsm, "texture8.png"));
        }
